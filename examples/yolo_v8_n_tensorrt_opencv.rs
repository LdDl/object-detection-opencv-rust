//! YOLOv8n with TensorRT backend + OpenCV for image I/O
//!
//! This example demonstrates the `tensorrt-opencv-compat` feature:
//! - OpenCV handles image loading (imread) and drawing
//! - TensorRT handles neural network inference
//!
//! Run with: cargo run --example yolo_v8_n_tensorrt_opencv --no-default-features --features "tensorrt-opencv-compat" --release
...
fn main() {
    ...
    // Warm up
    for _ in 0..10 {
        let _ = ModelTrait::forward(&mut model, &img, 0.25, 0.4).expect("Warm-up failed");
    }
    ...
    let mut result = None;
    for _ in 0..n_runs {
        result = Some(ModelTrait::forward(&mut model, &img, 0.25, 0.4).expect("Inference failed"));
    }
